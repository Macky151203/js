...
function Chat() {
  ...
  useEffect(()=>{
    const fetcheddata=JSON.parse(localStorage.getItem('chat'))
    if(fetcheddata){
      console.log(fetcheddata.data)
      setchat(fetcheddata.data)
    }
  })
  ...
  const handlesubmit= (e)=>{
    e.preventDefault()
    const newobj={role:name,msg:text,time:new Date().toLocaleTimeString()}
    let tempobj
    if(chat){
      tempobj=[...chat,newobj]
    }
    ...
    console.log(tempobj)
    let finaldata={room:roomId,data:tempobj}
  ...
  return (
    <div className="bg-slate-800 text-white min-h-screen relative">
      <div className="text-center text-xl shadow-md py-4 p-1">Realtimechat-{name}-Roomid-{roomId}</div>
